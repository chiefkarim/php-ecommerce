...
function CategoryProductList({ category }: { category: Category }): JSX.Element {
  const categoryId = category.slug === 'all' ? null : category.id;
...
export function CategoryPage(): JSX.Element {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 md:px-20 md:py-10">
      <CategoryProductList category={activeCategory} />
    </section>
  );
}
